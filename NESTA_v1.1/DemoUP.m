...
load('../eeg_sliced.mat'); load('../BernoulliSample.mat');
Phi=BernoulliSample(1:M,1:N);

% Discrete cosine transform basis
Psi = dct(diag(ones(N,1)));
iPsi = inv(Psi);

disp('Creating Data');
randn('state',2009); rand('state',2009);
Amatrix = Phi * Psi;    % no longer a projector
iAmatrix = iPsi * Phi';
x0 = double(data(1,:))';
b = Amatrix*x0;
...
x_reproject(T) = At\b;
    ...
    fprintf('SNR: %f', snr(x0,x0-x_reproject));